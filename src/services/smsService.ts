    ...
    for (const chunk of chunks) {
      ...
      try {
        ...
        smsLogs.unshift({
          ...
          recipients: chunk,
          message,
          status: result.code === 'ok' || result.code === '100' || result.code === '101' ? 'success' : 'failed', // Adjust based on actual success codes
          providerResponse: result
        });

        if (result.code !== 'ok' && result.code !== '100' && result.code !== '101' && !result.status?.toLowerCase().includes('success')) {
           // Arkesel sometimes returns different success indicators. 
           // Common success: { code: 'ok', ... } or { status: 'success' }
           // We'll assume non-ok is failure for now, but be lenient if needed.
           // If response has 'error' or 'Authentication Failed', throw.
        ...
        // If it's an auth error, stop. If network, maybe retry?
        // For now, we will collect errors and throw at the end if all failed.
