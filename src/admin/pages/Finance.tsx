...
import { DollarSign, Download, FileText, Send, TrendingUp, CreditCard, AlertCircle, Loader } from 'lucide-react';
...
export default function Finance() {
  ...
  const [transactions, setTransactions] = useState<(FinanceRecord & { member: { first_name: string; last_name: string } | null })[]>([]);
  ...
  async function fetchData() {
    try {
      setLoading(true);
      const [trxData, statsData, chartData] = await Promise.all([
          financeService.getTransactions(currentPage, itemsPerPage),
          financeService.getStats(),
          financeService.getChartData()
      ]);
      
      setTransactions(trxData.data);
      setTotalItems(trxData.count || 0);
      setStats(statsData);
                             ...
                             <div key={trx.id} className="flex items-center justify-between p-3 hover:bg-slate-50 rounded-lg transition-colors border border-slate-50">
                                 <div>
                                     <p className="font-medium text-slate-900 text-sm">
                                         {trx.member ? `${trx.member.first_name} ${trx.member.last_name}` : 'Unknown Member'}
                                     </p>
                                     <p className="text-xs text-slate-500 capitalize">{trx.type}</p>
                                 </div>
                                 <div className="text-right">
                                     <p className="font-bold text-slate-900 text-sm">GH₵ {trx.amount}</p>
